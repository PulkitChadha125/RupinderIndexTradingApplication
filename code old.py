...
def read_csv_to_dictionary(file_path):
    ...
    for index, row in df.iterrows():
        ...
        signal_dict[strategy_tag] = {'CALL': False, 'PUT': False, 'Target1': None, 'Target2': None, 'Target3': None,
                                     ...
                                     "S": False,
                                     "Count": 0, "expiry": None, "SlMove1": None, "SlMove2": None, "TP1QTY": None,
                                     "TP2QTY": None, "TP3QTY": None, "strategytag": None
            , "EXITQTY": None, 'Trade': False, "new_time": None, "alfoxsymbol": None, "ce_token": None,
                                     "pe_token": None, "runonce": False}

        start_time_str = data_dict['PRO1']['StartTime']

        start_time_str = datetime.strptime(start_time_str, '%H:%M').time()
        end_time_str = data_dict['PRO1']['StopTime']
    ...
    try:
        for strategy_tag, data in data_dict.items():
            if pd.notna(strategy_tag):
                ...
                if signal_dict[strategy_tag]["runonce"] == False:

                    if ExpieryContract == "MONTHLY":
                        ce_atm = f"{symbol}{Expiery_zerodha}{int(strike)}CE"
                        pe_atm = f"{symbol}{Expiery_zerodha}{int(strike)}PE"
                    if ExpieryContract == "WEEKLY":
                        ce_atm = f"{symbol}{expiery_zerodha_weekly}{int(strike)}CE"
                        pe_atm = f"{symbol}{expiery_zerodha_weekly}{int(strike)}PE"
                    ...
                    signal_dict[strategy_tag]["runonce"] = True

                print("Ce ATM: ", ce_atm)
                print("Pe ATM: ", pe_atm)

                df_ce = Zerodha_Integration.get_historical_data(sym=ce_atm, Token=signal_dict[strategy_tag]["ce_token"],
                ...
                df_pe = Zerodha_Integration.get_historical_data(sym=pe_atm, Token=signal_dict[strategy_tag]["pe_token"],
                                                                timeframe=timeframe, exp=expiryhistorical,
                ...
                vol_ce = df_pe['volume'].iloc[-n:].max()
                ...
                if (float(rsi_ce) > float(RSIValue) and float(price_ce) > float(vwap_ce) and float(oi_ce) < float(
                    ...
                    if CalculationType == "POINTS":
                        ...
                        signal_dict[strategy_tag]['SlMove1'] = float(signal_dict[strategy_tag]['Sl']) + float(SlMove1)
                        signal_dict[strategy_tag]['SlMove2'] = float(signal_dict[strategy_tag]['SlMove1']) + float(
                            SlMove2)
                    ...
                    Algofox.Buy_order_algofox(symbol=algofox_sym, quantity=int(EntryLotsize), instrumentType="OPTIDX",
                                              direction="BUY",
                                              product="MIS", strategy=stg, order_typ=OrderType, price=float(tradeprice),
                ...
                vol_pe = df_pe['volume'].iloc[-n:].max()
    ...
    try:
        for strategy_tag, data in data_dict.items():
            if pd.notna(strategy_tag):
                ...
                # print(f'{timestamp} signal_dict[strategy_tag]["Trade"]',signal_dict[strategy_tag]['Trade'])
                # print(f'{timestamp} signal_dict[strategy_tag]["new_time"]', signal_dict[strategy_tag]["new_time"])
                # print(f'{timestamp} signal_dict[strategy_tag]["S"] ', signal_dict[strategy_tag]['S'] )
                # print(f'{timestamp} signal_dict[strategy_tag]["T3"] ', signal_dict[strategy_tag]['T3'])
                #  call cycle
...
#
# def check_ltp():
#     sym= "BANKNIFTY23OCT43900CE"
#     Zerodha_Integration.get_ltp_option(sym)
