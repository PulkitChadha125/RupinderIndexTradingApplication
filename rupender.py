...
def write_to_order_logs(message):
    with open('C:\\Users\\PC\\Desktop\\Pulkit\\Ruperder project 1\\OrderLog.txt', 'a') as file:  # Open the file in append mode
        file.write(message + '\n')
def get_zerodha_credentials():
    delete_file_contents("C:\\Users\\PC\\Desktop\\Pulkit\\Ruperder project 1\\OrderLog.txt")
    credentials = {}
    try:
        df = pd.read_csv('C:\\Users\\PC\\Desktop\\Pulkit\\Ruperder project 1\\ZerodhaCredentials.csv')
...
df.to_csv("C:\\Users\\PC\\Desktop\\Pulkit\\Ruperder project 1\\Instruments.csv")
...
def read_csv_to_dictionary(file_path):
    ...
    for index, row in df.iterrows():
        ...
        signal_dict[strategy_tag] = {'CALL': False, 'PUT': False, 'Target1': None, 'Target2': None, 'Target3': None,
                                     ...
                                     "StopTime": None, "runonce": False,"processdata_call":None,"processdata_put":None}
...
data_dict = read_csv_to_dictionary("C:\\Users\\PC\\Desktop\\Pulkit\\Ruperder project 1\\TradeSettings.csv")
...
def process_data(data_dict):
    global NextEntryTime, pe_atm, ce_atm, signal_dict, ce_token, pe_token
    Algofoxid = credentials_dict.get('Algofoxid')
    Algofoxpassword = credentials_dict.get('Algofoxpassword')
    role = credentials_dict.get('role')
    ...
    try:
        for strategy_tag, data in data_dict.items():
            if pd.notna(strategy_tag):
                ...
                print(f"{current_time} Data collection started for {strategy_tag}")
                ...
                if entry_time <= datetime.strptime(current_time, "%H:%M:%S") <= exit_time:
                    ...
                    # sbse pehle ma od vol nikala suppose 100, last 3 candle vol to vol> maof vol* mutiplier
                    ...
                    if (float(rsi_ce) > float(RSIValue) and float(price_ce) > float(vwap_ce) and float(oi_ce) < float(
                        ...
                        if (float(rsi_ce_2) < float(RSIValue) or float(price_ce_2) < float(vwap_ce_2) or  float(oi_ce_2) > float(
                            maofoi_ce_2) ):
                            ...
                            if TradeExpieryContract == "MONTHLY":
                                tdExpiery = data['TradeExpiery']
                                tradeexp = zerodhahistorical(tdExpiery)
                                ce_strike_sym = Zerodha_Integration.get_sym(exp=tradeexp, strike=ce_strike, type="CE")
                    ...
                    if (float(rsi_pe) > float(RSIValue) and float(price_pe) > float(vwap_pe) and float(oi_pe) < float(
                            ...
                            signal_dict[strategy_tag]['CALL'] == False and signal_dict[strategy_tag]['PUT'] == False and
                            signal_dict[strategy_tag]['Trade'] == False):
                        if float(rsi_pe_2) < float(RSIValue) or float(price_pe_2) < float(vwap_pe_2) or float(oi_pe_2) > float(
                            maofoi_pe_2):
                            usedltp = zerodha_ltp_index(symbol)
                            ...
                            if TradeExpieryContract == "MONTHLY":
                                tdExpiery = data['TradeExpiery']
                                tradeexp = zerodhahistorical(tdExpiery)
                                pe_strike_sym = Zerodha_Integration.get_sym(exp=tradeexp, strike=pe_strike, type="PE")
    ...
    try:
        for strategy_tag, data in data_dict.items():
            if pd.notna(strategy_tag):
                ...
                # print("stop_time_str: ", stop_time_str)
                ...
                if stop_time_str is not None:
                    ...
                    if signal_dict[strategy_tag]['T3']== True and signal_dict[strategy_tag]['S']== True and current_time > stop_time and signal_dict[strategy_tag]['CALL']:
                ...
                if signal_dict[strategy_tag]['T1'] == True and signal_dict[strategy_tag]['S']== True and signal_dict[strategy_tag]['CALL'] == True:
                    tradeprice = Zerodha_Integration.get_ltp_option(signal_dict[strategy_tag]['PreviousString'])
                ...
                if signal_dict[strategy_tag]['T2'] == True and signal_dict[strategy_tag]['S']== True and signal_dict[strategy_tag]['CALL'] == True:
                    tradeprice = Zerodha_Integration.get_ltp_option(signal_dict[strategy_tag]['PreviousString'])
                    # print("tp_and_sl ltp:", tradeprice)
                ...
                if signal_dict[strategy_tag]['T3'] == True and signal_dict[strategy_tag]['S']== True and signal_dict[strategy_tag]['CALL'] == True:
                    tradeprice = Zerodha_Integration.get_ltp_option(signal_dict[strategy_tag]['PreviousString'])
                    # print("tp_and_sl ltp:", tradeprice)
                ...
                if signal_dict[strategy_tag]['S'] == True and signal_dict[strategy_tag]['CALL'] == True:
                    ...
                    if float(tradeprice) <= float(signal_dict[strategy_tag]['Sl']):
                        ...
                        #  PUT  cycle
                if signal_dict[strategy_tag]['T1'] == True and signal_dict[strategy_tag]['S']== True and signal_dict[strategy_tag]['PUT'] == True:
                    tradeprice = Zerodha_Integration.get_ltp_option(signal_dict[strategy_tag]['PreviousString'])
                    # print("tp_and_sl ltp:", tradeprice)
                    if float(tradeprice) >= float(signal_dict[strategy_tag]['Target1']):
                        Algofox.Sell_order_algofox(symbol=signal_dict[strategy_tag]["alfoxsymbol"],
                ...
                if signal_dict[strategy_tag]['T2'] == True and signal_dict[strategy_tag]['S']== True and signal_dict[strategy_tag]['PUT'] == True:
                    tradeprice = Zerodha_Integration.get_ltp_option(signal_dict[strategy_tag]['PreviousString'])
                    # print("tp_and_sl ltp:", tradeprice)
                    if float(tradeprice) >= float(signal_dict[strategy_tag]['Target2']):
                        Algofox.Sell_order_algofox(symbol=signal_dict[strategy_tag]["alfoxsymbol"],
                ...
                if signal_dict[strategy_tag]['T3'] == True and signal_dict[strategy_tag]['S']== True and signal_dict[strategy_tag]['PUT'] == True:
                    tradeprice = Zerodha_Integration.get_ltp_option(signal_dict[strategy_tag]['PreviousString'])
                    # print("tp_and_sl ltp:", tradeprice)
                    if float(tradeprice) >= float(signal_dict[strategy_tag]['Target3']):
                        Algofox.Sell_order_algofox(symbol=signal_dict[strategy_tag]["alfoxsymbol"],
